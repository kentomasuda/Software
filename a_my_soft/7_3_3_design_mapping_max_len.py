...
def process_sections(sections, net, added_nodes):
    ...
    for section in sections:
        ...
        if source_type == target_type:
            ...
            # 異なる製品タイプ
            net.add_edge(source_node, target_node, title=edge_content, color="green", length=200, width=3)
...
for perm in all_permutations_5:
    ...
    # 赤＆緑のエッジ最大長
    max_len = calc_max_red_green_edge_length(tmp_net)
    print(max_len)
...
# 最良結果を表示
print("=== 赤＆緑エッジの最大長が最も短い読み込み順(円順列) ===")
for i, p in enumerate(best_order, start=1):
    print(f"{i}: {p}")
print("最小の最大長:", best_max_len)

# 最終的なネットワークをHTML出力
best_network.show("network33.html")
print("network33.html に出力しました。")
