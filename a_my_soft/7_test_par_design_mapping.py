# -*- coding: utf-8 -*-
"""
Created on Wed Jan 15 19:00:33 2025
...
def process_sections(sections):
    ...
    for section in sections:
        ...
        if source_type == target_type:
            # 中間ノードがある
            middle_node = f"middle_{source_node}_{target_node}"
            net.add_edge(source_node, middle_node, title=edge_content, color="black", length=5)
            net.add_edge(middle_node, target_node, title=edge_content, color="black", arrows="to", length=5)
            ...
            # 異なる製品タイプ
            net.add_edge(source_node, target_node, title=edge_content, color="black", length=200)
...
def process_intermediate_edges(file_path):
    ...
    for i in range(0, len(lines), 5):
        ...
        # ノード追加（存在しない場合のみ）
        for node, typ, year, color in [
            (src1_node, src1_type, src1_year, src1_color),
            (tgt1_node, tgt1_type, tgt1_year, tgt1_color),
            (src2_node, src2_type, src2_year, src2_color),
            (tgt2_node, tgt2_type, tgt2_year, tgt2_color),
        ]:
            if node not in added_nodes:
                title = node_titles_dict.get(node, "")
                net.add_node(
                    node,
                    label=f"{typ}\n{year}",
                    title=title,
