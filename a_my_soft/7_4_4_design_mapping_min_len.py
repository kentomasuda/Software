# -*- coding: utf-8 -*-
"""
Created on Thu Jan 16 00:39:31 2025
...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 10 16:36:47 2025
...
def process_sections(sections, net, added_nodes):
    ...
    for section in sections:
        ...
        if source_type == target_type:
            ...
            # 異なる製品タイプ
            net.add_edge(source_node, target_node, title=edge_content, color="green", width=4,length=200)
...
def calc_min_red_green_edge_length(network):
    """
    ...
    # エッジが一つもなかった場合など、min_dist が inf のままなら 0 を返すなどの対処が可能
...
# ここから「赤＆緑の最大長が最小となるパターンを探索」するメイン処理
####################################################
...
best_network = None
best_min_len = -float('inf')  # 最小長の最大値を探すために負の無限大で初期化
best_order = None
...
for perm in all_permutations_5:
    ...
    # 赤＆緑のエッジ最小長を計算
    min_len = calc_min_red_green_edge_length(tmp_net)
    print(min_len)
    
    # より大きい“最小長”なら更新
    if min_len > best_min_len:
        best_min_len = min_len
        best_order = current_order
        best_network = tmp_net

# 最良結果を表示
print("=== 赤＆緑エッジの最小長の最大値となる読み込み順(円順列) ===")
for i, p in enumerate(best_order, start=1):
    print(f"{i}: {p}")
print("最大の最小長:", best_min_len)

# 最終的なネットワークをHTML出力
best_network.show("network44.html")
print("network44.html に出力しました。")

print(len(all_permutations_5))
