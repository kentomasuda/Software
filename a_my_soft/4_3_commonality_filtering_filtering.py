# -*- coding: utf-8 -*-
"""
Created on Fri Jan 10 14:05:41 2025
...
file_path = 'filtered_high_similarity_pairs_with_descriptions.txt'  # 実際のデータファイルのパスを指定
...
for section in sections:
    ...
    section_info_list.append({
        ...
        "device1": device1,
        "device2": device2,
        "features": {feature1, feature2}  # 特徴をセットで保持
    })
...
for i in range(len(section_info_list)):
    for j in range(i + 1, len(section_info_list)):
        ...
        if sec_i["device1"] == sec_j["device1"] and sec_i["device2"] == sec_j["device2"]:
            # 特徴のいずれかが一致しているか確認
            if sec_i["features"] & sec_j["features"]:  # 共通の特徴があるか
                # 古い方を残す（年の合計が小さい方）
...
output_file_path = 'filtered_filtered_high_similarity_pairs_with_descriptions.txt'  # 出力ファイル名

with open(output_file_path, 'w', encoding='utf-8') as outfile:
    content = "\n\n".join(
        "\n".join(sec["section"].splitlines()[:-1])  # 最後の1行を除外して再結合.同じ機能が2行あるので削除
        for sec in filtered_sections
    )
